...
public class enemySpawner : MonoBehaviour
{
   ...
   Vector3 GetRandomPosition(bool withinCamera)
   {
       ...
       //this will be used later so enemies just spawn in the camera view, so even in fog we can spawn enemies that aren't too far away!
       ...
       position += target.transform.position; //This focuses the spawn radius to be around the player, i.e player = center of circle
   ...
   enemySCR AddEnemy(Vector3 position)
   {
       ...
       //We instantiate a new newEnemy, this is fairly staright forward, although we could change vector3.up to something else for grounded enemies
       ...
       //enemyScript.gameArea = gameArea; //Placing the gameArea(plane) to the enemyPrefab script NOT NEEDED
